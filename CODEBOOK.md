...
Seeded entirely from the *Archives of California* calendar (leaf-verified catalog of the Savage transcripts, archivesofcalifornia.com) by a documented harvester (scripts/harvest.py + merge.py). **Status (updated 2026-07-27):** after a roster-wide corroboration audit against the C-A records, Bancroft (all 7 Hist.Cal. vols, local), and Ogden, **372 of 399 ships are `reviewed`** (identity corroborated by a reliable source); the remaining **27 stay `draft`** — OCR garbles, unidentified names, and place/person-name suspects still needing a source check (see REVIEW-QUEUE.md). `verified` (leaf-checked) is still pending. Pending source families, in order: Bancroft HoC narrative 1769–1824 → Ogden 1941 appendix + Archer 1973 → the Russian record (Gibson–Istomin, Khlebnikov, Tikhmenev, Ivashintsov) → Howay/Cook cross-checks. Absence of a row ≠ absence of a ship: contraband is under-recorded by design — that asymmetry is a finding, not a flaw.
